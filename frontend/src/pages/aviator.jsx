...
function BetPanel({
  ...
  useEffect(() => {
    ...
    const handler = (data) => {
      ...
      onBalanceChange({ balance: data.balance, bonus_balance: data.bonus_balance });
    };
  ...
  const placeBetCore = async (betAmountValue, betMode, autoCashoutTarget, isAuto) => {
    ...
    try {
      ...
      onBalanceChange({ balance: res.data.balance, bonus_balance: res.data.bonus_balance });
    } catch (err) {
...
export function GameBoard({
  ...
  // externally-supplied balance/setter in that case. For local-wallet
  // play, the displayed number is balance + bonus_balance combined - the
  // total actually spendable on Aviator (unlike Bingo, which is
  // deposit-only and only ever shows plain balance) - so a user playing
  // entirely on coupon money (balance = 0, bonus_balance > 0) sees a real
  // number here instead of a misleading "0 ETB" while their bets are
  // genuinely being funded and working behind the scenes.
  const displayBalance =
    externalBalance !== undefined ? externalBalance : Number(user?.balance || 0) + Number(user?.bonus_balance || 0);
  ...
  const handleBalanceChange = (update) => {
    if (typeof update === 'number') {
      // Legacy shape - a bare balance number (still used by the
      // operator-embedded seamless-wallet flow, which has no concept of
      // bonus_balance at all).
      setBalance(update);
    } else if (update && typeof update === 'object') {
      // { balance, bonus_balance } shape - what the local-wallet bet/
      // cashout/cancel responses actually return. setBalance (aka
      // updateBalance/onExternalBalanceChange) accepts this same shape
      // directly - see app-shell.jsx's updateBalance, which merges
      // whichever fields are present into the user object rather than
      // requiring a plain number. Passing the whole object through in one
      // call (rather than re-fetching /wallet/balance, which used to be
      // the fallback here) is also what actually fixes bonus_balance
      // being silently dropped - that endpoint's response never included
      // it either.
      setBalance(update);
    }
  };

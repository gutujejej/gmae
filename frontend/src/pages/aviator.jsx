 ...
 *  Synthesized sound effects using the Web Audio API, plus file-based
 *  background music + start/cashout sounds added on top.
 * ==================================================================== */

let audioContext = null;
let muted = false;

// Background music loop (frontend/public/loop.mp3) - a single shared
// <audio> element reused across mounts so the music doesn't restart or
// double up if the Aviator screen unmounts/remounts.
let bgmElement = null;

function getBgmElement() {
  if (!bgmElement) {
    bgmElement = new Audio('/loop.mp3');
    bgmElement.loop = true;
    bgmElement.volume = 0.35;
  }
  return bgmElement;
}

// Browsers block audio with sound from autoplaying until the user has
// interacted with the page at least once. Call this from a user gesture
// (e.g. the first bet, or the mute button itself) to satisfy that and
// start the loop. Safe to call repeatedly - a no-op if already playing.
function startBackgroundMusic() {
  const el = getBgmElement();
  el.muted = getMuted();
  if (el.paused) {
    el.play().catch(() => {
      // Autoplay was blocked (no user gesture yet) - this is expected on
      // first load and is retried on the next user interaction (bet
      // placed, mute button tapped, etc.) rather than surfaced as an error.
    });
  }
}
...
// One-shot file-based sound effects (start of round, cash-out). Each call
// creates a fresh Audio instance so overlapping plays (e.g. two quick
// cash-outs) don't cut each other off.
function playStartSound() {
  if (getMuted()) return;
  const el = new Audio('/start.mp3');
  el.volume = 0.6;
  el.play().catch(() => {});
}

function playCashoutFileSound() {
  if (getMuted()) return;
  const el = new Audio('/cashout.mp3');
  el.volume = 0.6;
  el.play().catch(() => {});
}

function playCrashFileSound() {
  if (getMuted()) return;
  const el = new Audio('/crash.mp3');
  el.volume = 0.6;
  el.play().catch(() => {});
}
...
// feel more rewarding. Layered with the cashout.mp3 file sound.
function playCashOutSound(multiplier = 1) {
  if (getMuted()) return;
  playCashoutFileSound();
...
// when the plane crashes. Layered with the crash.mp3 file sound.
...
function FlightStage({ phase, multiplier, crashPoint, bettingSecondsLeft }) {
  ...
  // Remembers the furthest the "camera" has panned so far this round.
  // offsetX only ever gets more negative (pans left) and offsetY only
  // ever gets more positive (pans up) as the plane climbs - never the
  // reverse. Without this, recomputing the anchor-relative offset fresh
  // on every tick let small server-timing jitter in the multiplier flip
  // which side of the anchor the raw position landed on, snapping the
  // whole curve+plane backward and forward - the "glitch" this fixes.
  ...
  useEffect(() => {
    ...
    if (phase !== 'flying' && phase !== 'ended') {
      ...
      // A fresh round is starting (or about to) - reset the pan so the
      // next flight starts from the same anchored position instead of
      // inheriting the previous round's fully-panned camera.
    ...
    const candidateOffsetX = Math.min(0, anchorX - rawTipX);
    const candidateOffsetY = Math.max(0, anchorY - rawTipY);

    // Only ever pan further in the same direction - never snap back.
    const offsetX = Math.min(panRef.current.offsetX, candidateOffsetX);
    const offsetY = Math.max(panRef.current.offsetY, candidateOffsetY);
    panRef.current = { offsetX, offsetY };
...
const QUICK_AMOUNTS = [4, 40, 80, 400];
...
function BetPanel({
  ...
  const isOperatorSession = endpointPrefix !== '/game';
  const autoCashedOutEvent = isOperatorSession ? 'operator_bet:auto_cashed_out' : 'bet:auto_cashed_out';
  ...
  // the ref instead of stale closed-over state. Auto-bet-next-round is a
  // local-wallet-only feature (it depends on a persisted per-user setting
  // tied to a real account), so it's skipped entirely for operator
  // sessions - see the hidden checkbox further down too.
  useEffect(() => {
    if (phase === 'betting') {
      setStatus('idle');
      ...
      setPayout(null);
      setError(null);
      if (!isOperatorSession && autoBetNextRound) placeBetWithLatestValues();
  ...
  useEffect(() => {
    ...
    socket.on(autoCashedOutEvent, handler);
    return () => socket.off(autoCashedOutEvent, handler);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [socket, slot]);
  ...
  const placeBetCore = async (betAmountValue, betMode, autoCashoutTarget, isAuto) => {
    ...
    if (betAmountValue < MIN_BET_BIRR) {
      if (!isAuto) setError(`Minimum bet is ${MIN_BET_BIRR} ETB`);
      return;
    }
    ...
    setSubmitting(true);
    try {
      const res = await apiClient.post(`${endpointPrefix}/bet`, payload);
  ...
  const cancelBet = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await apiClient.post(`${endpointPrefix}/cancel-bet`, { slot });
      setStatus('idle');
      setBetAmount(null);
      onBalanceChange(res.data.balance);
    } catch (err) {
  ...
  const cashOut = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await apiClient.post(`${endpointPrefix}/cashout`, { slot });
  ...
  const toggleAutoBetNextRound = async (checked) => {
    ...
    try {
      await apiClient.put(`${endpointPrefix}/auto-bet/${slot}`, {
        enabled: checked,
        amount,
  ...
  const step = (delta) => {
    ...
    setAmount((prev) => Math.max(MIN_BET_BIRR, Math.round((prev + delta) * 100) / 100));
  };
...
function ActionButton({ phase, status, amount, betAmount, cashoutMultiplier, payout, potentialPayout, submitting, onPlace, onCancel, onCashOut, currencyLabel }) {
  if (phase === 'betting') {
    if (status === 'placed') {
      return (
        <button className="bet-btn state-placed" onClick={onCancel} disabled={submitting}>
          <span className="bet-btn-top">Cancel</span>
          <span className="bet-btn-sub">{betAmount?.toFixed(2)} {currencyLabel}</span>
        </button>
      );
    }
    return (
      <button className="bet-btn state-idle" onClick={onPlace} disabled={submitting}>
        ...
        <span className="bet-btn-sub">
          {amount?.toFixed(2)}
          <span className="cur-tag">{currencyLabel}</span>
        </span>
      </button>
    );
  }
    ...
    if (status === 'won') {
      return (
        <button className="bet-btn state-won" disabled>
          ...
          <span className="bet-btn-sub">+{payout?.toFixed(2)} {currencyLabel}</span>
        </button>
      );
    }
    if (status === 'cashout' || status === 'placed') {
      return (
        <button className="bet-btn state-cashout" onClick={onCashOut} disabled={submitting}>
          ...
          <span className="bet-btn-sub">
            {potentialPayout ? `${potentialPayout} ${currencyLabel}` : `${amount?.toFixed(2)} ${currencyLabel}`}
          </span>
        </button>
      );
    }
  ...
  if (status === 'lost') {
    return (
      <button className="bet-btn state-lost" disabled>
        ...
        <span className="bet-btn-sub">{amount?.toFixed(2)} {currencyLabel}</span>
      </button>
    );
  }
...
function MuteButton() {
  ...
  const toggle = () => {
    ...
    setMuted(next);
    setMutedState(next);
    // This click is a user gesture, so if the music loop was blocked by
    // the browser's autoplay policy on mount, this reliably unlocks it -
    // unmuting will both un-silence it and (if it never started) start it.
...
function TopBar({ balance, brandLabel, currencyLabel }) {
  return (
    <div className="aviator-topbar">
      <div className="aviator-brand">{brandLabel}</div>
      <div className="aviator-topbar-right">
        <span className="aviator-balance">{Number(balance || 0).toFixed(2)} {currencyLabel}</span>
        <MuteButton />
      </div>
    </div>
  );
...
export function GameBoard({
  ...
  endpointPrefix = '/game',
  externalBalance,
  onExternalBalanceChange,
  brandLabel = 'Aviator',
  currencyLabel = 'ETB',
}) {
  const { user, updateBalance } = useAuth();
  // Operator-embedded sessions have no AuthContext user - fall back to the
  // externally-supplied balance/setter in that case.
  const displayBalance = externalBalance !== undefined ? externalBalance : user?.balance;
  const setBalance = onExternalBalanceChange || updateBalance;
  ...
  useEffect(() => {
    ...
    // Best-effort start - browsers block audio until the user has
    // interacted with the page at least once, so this may not actually
    // start playing yet. It's retried from the mute button and from the
    // first bet placed (both are user gestures), which reliably unlocks it.
    startBackgroundMusic();
    ...
    socket.on('round:tick', (data) => {
      // Guard against the multiplier ever visually regressing - a
      // late-arriving or duplicate tick with a lower value would
      // otherwise snap the plane/curve backward for a frame before the
      // next correct tick catches back up.
  ...
  const handleBalanceChange = (newBalance) => {
    ...
    } else if (endpointPrefix === '/game') {
      // Local-wallet play only: re-fetch the authoritative balance when a
      // route didn't return one inline. Operator sessions always get
      // their balance back directly from the operator's callback
      // response, so there's nothing to re-fetch here for them.
      apiClient.get('/wallet/balance').then((res) => setBalance(res.data.balance));
    }
  };
  ...
  return (
    <div className="card aviator-card">
      ...
      <TopBar balance={displayBalance} brandLabel={brandLabel} currencyLabel={currencyLabel} />

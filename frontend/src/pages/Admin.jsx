...
function OperatorsTab() {
  ...
  const toggleActive = async (operator) => {
    try {
      ...
      alert(err.response?.data?.error || 'Failed to update operator');
    }
  };

  const handleRegenerate = async (operator) => {
    if (!window.confirm(`Regenerate the API secret for ${operator.name}? The old secret will stop working immediately.`)) return;
    try {
                ...
                data.operators.map((op) => (
                  <tr key={op.id}>
                    ...
                    <td style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      <button className="btn btn-outline" onClick={() => handleRegenerate(op)}>
                        Regenerate Secret
                      </button>
                      <button className="btn btn-outline" onClick={() => handleDelete(op)}>
                        Delete
                      </button>
                    </td>
                  </tr>
...
function PhotoBroadcastForm() {
  ...
  const handleSend = async (e) => {
    ...
    if (!window.confirm('Send this image to every Telegram user right now?')) return;
  ...
  return (
    <div>
      <form onSubmit={handleSend} className="admin-section-card">
        ...
        <button className="btn btn-primary" type="submit" disabled={sending} style={{ marginTop: 12 }}>
          {sending ? 'Sending...' : 'Send Photo Broadcast'}
        </button>
      </form>
...
function BroadcastTab() {
  ...
  const handleSend = async (e) => {
    ...
    if (!window.confirm('Send this message to every Telegram user right now?')) return;

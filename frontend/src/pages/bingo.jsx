...
export default function Bingo() {
  ...
  useEffect(() => {
    ...
    socket.on('bingo:waiting', (data) => {
      ...
      setStakeAmount(data.stake_amount);
      setCalledNumbers([]);
      setMyCartelas([]);
      setRoundResult(null);
      setJoinDeadline(Date.now() + data.join_seconds * 1000);
    ...
    getBingoState()
      .then((res) => {
        ...
        setStatus(d.status);
        setStakeAmount(d.stake_amount);
        setCalledNumbers(d.called_numbers);
        setMyCartelas(d.my_cartelas);
        setJoinDeadline(d.join_deadline);
        setPlayersInRound({ players: d.players, cartelas: d.cartelas_in_play });
      })
      .catch(() => {});
...
function RoundResultBanner({ result, currentUserId }) {
  const iWon = result.winners.some((w) => w.user_id === currentUserId);
  ...
  return (
    <>
      <div style={{ fontSize: 15, color: iWon ? '#4ade80' : '#f2f2f4', fontWeight: 700 }}>
        {iWon ? '🎉 You won!' : 'Round winner' + (result.winners.length > 1 ? 's' : '')}
      </div>
      ...
      <div style={{ fontSize: 18, fontWeight: 800, color: '#f7b955', marginTop: 8 }}>
        {result.payout_each.toFixed(2)} ETB {result.winners.length > 1 ? 'each' : ''}
      </div>

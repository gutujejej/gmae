...
adminRouter.post('/broadcast', broadcastValidation, handleValidation, async (req, res, next) => {
  try {
    ...
    const telegramIds = rows.map((r) => r.telegram_id);
    ...
    const result = await broadcastMessage(telegramIds, req.body.message.trim());
    ...
    res.json({ message: 'Broadcast sent', ...result, total: telegramIds.length });
  } catch (err) {
    next(err);
  }
});

  ...
  try {
    const { rows } = await query(
      `SELECT key, value FROM platform_settings
       WHERE key IN ('signup_bonus_birr', 'referral_commission_percent', 'game_logo_url')`
    );
...
/* ------------------------------------------------------------------ */
/*  Telegram broadcast                                                  */
/*  Sends a text message to every user who registered via the Telegram  */
/*  bot (i.e. has a telegram_id on file). Actual sending happens in     */
/*  telegram-bot.js - this route just queues it up and reports back how */
/*  many recipients it went to (or attempted).                          */
/* ------------------------------------------------------------------ */
...
adminRouter.post('/broadcast', broadcastValidation, handleValidation, async (req, res, next) => {
  try {
    ...
    // Migration-transition only: also reach users who haven't opened the
    // Mini App since the database switch and so only exist on the OLD
    // database still - see telegram-bot.js's handleStart, which is what
    // actually creates their new-DB account the moment they tap through.
    // A telegram_id already covered above (they've already migrated) is
    // never double-counted, since this is a Set keyed by telegram_id.
    if (oldPool) {
      try {
    ...
    const telegramIdList = Array.from(telegramIds);

...
app.use(
  express.json({
    limit: '100kb',
    // Captures the exact raw bytes of the request body before JSON
    // parsing, needed so /api/auth/telegram-register can verify the
    // Telegram bot's HMAC signature over the untouched payload - signing
    // the re-serialized/parsed JSON wouldn't reliably match what the
    // sender actually signed (key order, whitespace, etc. can differ).
    verify: (req, res, buf) => {
...
app.use('/api/admin/operators', operatorAdminRouter);
app.use('/api/operator', operatorPublicRouter);
app.use('/api/operator-game', operatorGameRouter);
...
async function start() {
  ...
  server.listen(port, () => {
    ...
    // Started after listen() so telegram-register calls the bot makes
    // against this same process (via SELF_API_URL) always reach a server

...
// Sends a photo with an optional caption. First call uploads the actual
// image bytes to Telegram (multipart/form-data - telegramCall() only
// handles JSON, so this builds its own request); Telegram's response
// includes a file_id that can be reused for every subsequent chat in a
// broadcast without re-uploading the file each time, which is both much
// faster and avoids needing to persist the image anywhere on our own
// disk (Railway's filesystem is ephemeral, so nothing is saved locally -
// the buffer from the admin's upload goes straight to Telegram and is
// then discarded).
async function sendPhotoUpload(chatId, imageBuffer, filename, caption) {
  const form = new FormData();
  form.append('chat_id', String(chatId));
  if (caption) form.append('caption', caption);
  form.append('photo', new Blob([imageBuffer]), filename || 'broadcast.jpg');

  const res = await fetch(`${TELEGRAM_API}/sendPhoto`, { method: 'POST', body: form });
  const data = await res.json().catch(() => ({}));
  if (!data.ok) {
    logger.warn('[telegram-bot] sendPhoto (upload) failed', { description: data.description });
  }
  return data;
}

function sendPhotoByFileId(chatId, fileId, caption) {
  return telegramCall('sendPhoto', { chat_id: chatId, photo: fileId, caption, parse_mode: 'HTML' });
}
...
async function handleStart(message) {
  ...
  // Try a phone-less login first - telegram-register logs an EXISTING
  // telegram_id straight in without requiring phone again (see
  // users.js), so a user who has already been through registration once
  // (on this database) skips the phone-share prompt entirely and goes
  // straight to the Play button. Only a genuinely new telegram_id (never
  // registered on THIS database - including someone who only ever
  // registered on the old one, pre-migration) gets 'phone_required' back
  // and is asked to share their number, per the migration-transition
  // design: opening the Mini App is what creates their fresh new-DB
  // account, not anything that happened on the old database.
  ...
  if (result.error === 'phone_required') {
    if (referralCode) pendingReferralByChat.set(chatId, referralCode);
  ...
  // Existing user - already fully set up, straight to Play.
...
// Broadcasts an image (with optional caption) to every given
// telegram_id. Uploads the image once, to the FIRST recipient, then
// reuses the file_id Telegram hands back for every remaining recipient -
// same image bytes, no re-upload, and it's what Telegram itself
// recommends for sending one file to many chats.
async function broadcastPhoto(telegramIds, imageBuffer, filename, caption) {
  if (!BOT_TOKEN) {
    return { sent: 0, failed: telegramIds.length, error: 'Bot is not configured (TELEGRAM_BOT_TOKEN missing)' };
  }
  ...
  const [firstId, ...restIds] = telegramIds;

  const firstResult = await sendPhotoUpload(firstId, imageBuffer, filename, caption);
  if (firstResult.ok) {
    sent += 1;
    fileId = firstResult.result?.photo?.slice(-1)[0]?.file_id || null;
  } else {
    failed += 1;
  }

  // If the upload itself failed, there's no file_id to reuse - nothing
  // further can be sent, so every remaining recipient counts as failed
  // rather than silently being skipped.
  if (!fileId) {
    return { sent, failed: failed + restIds.length };
  }

  for (const telegramId of restIds) {
    await new Promise((r) => setTimeout(r, 40));
    const result = await sendPhotoByFileId(telegramId, fileId, caption);
    if (result.ok) {
      sent += 1;
    } else {
      failed += 1;
...
module.exports = { startTelegramBot, stopTelegramBot, broadcastMessage, broadcastPhoto, checkRequiredMembership };

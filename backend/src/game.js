...
async function processAutoCashouts(multiplier) {
  ...
  for (const bet of dueBets) {
    ...
    try {
      ...
      // A win ALWAYS credits balance, never bonus_balance - see the
      // matching comment in the manual /cashout route above for why.
      await client.query(
        `UPDATE bets SET status = 'cashed_out', cashed_out_at = $1, payout = $2 WHERE id = $3`,
        [cashoutMultiplier, payoutCents, lockedBet.id]
      );
      const { rows: autoUpdatedRows } = await client.query(
        'UPDATE users SET balance = balance + $1 WHERE id = $2 RETURNING balance, bonus_balance',
        [payoutCents, lockedBet.user_id]
      );
      await client.query(
      ...
      broadcast('bet:auto_cashed_out', {
        ...
        balance: autoUpdatedRows[0].balance / 100,
        bonus_balance: autoUpdatedRows[0].bonus_balance / 100,
      });
    } catch (err) {
      await client.query('ROLLBACK');
  ...
  for (const bet of losingBets) {
    ...
    try {
      ...
      if (bet.bonus_amount > 0) {
        ...
        const { rows: wageringRows } = await client.query(
          'UPDATE users SET wagering_required = GREATEST(wagering_required - $1, 0) WHERE id = $2 RETURNING wagering_required, bonus_balance',
          [bet.bonus_amount, bet.user_id]
        );

        // Same auto-conversion as the bet-placement route: once wagering
        // is fully met, whatever's left in bonus_balance permanently
        // becomes real balance - a losing bet can be the one that
        // finally crosses the wagering line just as easily as a winning
        // one, so this has to be checked here too, not only on placement.
        if (wageringRows[0].wagering_required <= 0 && wageringRows[0].bonus_balance > 0) {
          await client.query(
...
router.post('/bet', requireAuth, betValidation, handleValidation, async (req, res, next) => {
  ...
  try {
    ...
    const { rows: updatedRows } = await client.query(
      'UPDATE users SET balance = balance - $1, bonus_balance = bonus_balance - $2 WHERE id = $3 RETURNING balance, bonus_balance, wagering_required',
      [fromBalance, fromBonus, req.user.id]
    );
    let finalBalance = updatedRows[0].balance;
    let finalBonusBalance = updatedRows[0].bonus_balance;

    // Any part of this stake funded by coupon money counts toward the
    // 100x wagering requirement, win or lose - the bet being PLACED is
    // what counts, not the outcome. Once wagering_required reaches 0,
    // whatever's left in bonus_balance permanently converts into real,
    // spendable/withdrawable balance - from that point on it's no
    // different from deposited money (usable on Bingo, withdrawable,
    // etc.), matching the "coupon money that has finished wagering
    // behaves as deposit money" rule.
    if (fromBonus > 0) {
      const newWageringRequired = Math.max(0, updatedRows[0].wagering_required - fromBonus);
      if (newWageringRequired <= 0 && finalBonusBalance > 0) {
        const convertedRows = await client.query(
          `UPDATE users
           SET wagering_required = 0, balance = balance + bonus_balance, bonus_balance = 0
           WHERE id = $1
           RETURNING balance, bonus_balance`,
          [req.user.id]
        );
        finalBalance = convertedRows[0].balance;
        finalBonusBalance = convertedRows[0].bonus_balance;
      } else {
        await client.query('UPDATE users SET wagering_required = $1 WHERE id = $2', [
          newWageringRequired,
          req.user.id,
        ]);
      }
    ...
    res.status(201).json({
      ...
      bonus_balance: finalBonusBalance / 100,
    });
  } catch (err) {
    await client.query('ROLLBACK');
    next(err);
...
router.post('/cancel-bet', requireAuth, cancelBetValidation, handleValidation, async (req, res, next) => {
  ...
  try {
    ...
    // lock just by placing then cancelling a bet. Symmetrically, the
    // wagering credit the bet route gave for its bonus-funded portion
    // must be un-done here too - otherwise repeatedly placing and
    // cancelling a coupon-funded bet would burn down wagering_required
    // for free, without the money ever actually being at risk.
    ...
    const { rows: updatedRows } = await client.query(
      `UPDATE users
       SET balance = balance + $1,
           ...
           wagering_required = LEAST(wagering_required + $2, wagering_target_total)
       WHERE id = $3
       RETURNING balance, bonus_balance`,
...
router.post('/cashout', requireAuth, cashoutValidation, handleValidation, async (req, res, next) => {
  ...
  try {
    ...
    // A win ALWAYS credits balance (the deposit wallet), never
    // bonus_balance - even if part or all of the stake was funded by
    // coupon money. Coupon money (bonus_balance) only ever decreases; it
    // is never replenished by winnings. This is what lets coupon-funded
    // wagering progress mean something concrete: the money is genuinely
    // spent down, and once cumulative coupon-funded betting reaches the
    // 100x wagering target, whatever's left in bonus_balance unlocks and
    // becomes real, spendable/withdrawable balance (see wagering_required
    // reaching 0 below).

...
/*    node src/scripts/generate-bingo-cartelas.js                         */
/* ====================================================================== */
...
async function run() {
  ...
  const { rows: existingRows } = await query('SELECT COUNT(*)::int AS count FROM bingo_cartelas');
  if (existingRows[0].count > 0) {
    console.log(`bingo_cartelas already has ${existingRows[0].count} cards - nothing to do.`);
    console.log('Delete the existing rows first if you really want to regenerate them.');
    await pool.end();
    return;
  }

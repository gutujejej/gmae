...
// pending[`${botId}:${telegramId}`] = { step: 'awaiting_deposit_amount' | 'awaiting_deposit_reference' | 'awaiting_withdraw_amount' | 'awaiting_withdraw_phone', amount? }
  ...
  async function handleWalletMessage(bot, row, msg, user) {
    ...
    if (state) {
      if (state.step === 'awaiting_deposit_amount') {
        ...
        pending.set(key, { step: 'awaiting_deposit_reference', amount });
        await bot.sendMessage(chatId,
          `Now send the *transaction/reference code* from the Telebirr SMS or receipt for this payment.`,
          { parse_mode: 'Markdown' });
        return true;
      }
      ...
      if (state.step === 'awaiting_deposit_reference') {
        ...
        if (!reference || reference.length < 4) {
          await bot.sendMessage(chatId, 'Please send the Telebirr transaction/reference code (usually a short code from the SMS), or type /cancel.');
          return true;
        }
        pending.delete(key);
        const { error } = await db.from('deposits').insert({
          bot_id: row.id, telegram_id: msg.from.id, amount: state.amount, note: reference,
        });
        if (error) {
    ...
    if (text === MENU_EXTRA.deposit) {
      ...
      const phone = String(row.deposit_phone || '0930008319');
      await bot.sendMessage(chatId,
        `💵 *Deposit*\n\n` +
        `1️⃣ Open Telebirr and send any amount to:\n\`${phone}\`\n(tap the number to copy it)\n\n` +
        `2️⃣ Come back here and reply with the amount you sent (numbers only, e.g. \`100\`)\n\n` +
        `3️⃣ You'll then be asked for the transaction/reference code from the Telebirr SMS\n\n` +
        `4️⃣ Wait for admin approval — your balance updates automatically once approved.`,
        { parse_mode: 'Markdown' });
      return true;
    }

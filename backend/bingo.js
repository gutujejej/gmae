...
module.exports = function attachBingo(app, db, { requireAdmin, miniAuth, mainMenuFor, sendMessage }) {
  ...
  // The most recent round that finished with a winner in the last few
  // seconds. Used to show every player the winner announcement.
  const RESULT_WINDOW_MS = 10_000;
  async function getRecentFinishedRound(botId, stake) {
    const since = new Date(Date.now() - RESULT_WINDOW_MS).toISOString();
    const { data, error } = await db.from('bingo_rounds')
      .select('*').eq('bot_id', botId).eq('stake', stake).eq('status', 'finished')
      .not('winner_telegram_id', 'is', null).gte('finished_at', since)
  ...
  async function startRound(roundId) {
    ...
    const handle = setInterval(async () => {
      try {
        ...
        // auto-mark this number on every card in the round, and look for a win
        ...
        for (const card of cards) {
          ...
          // Judge from the server's own called list, same as the manual claim.
          const calledSet = new Set(called);
          const serverMarked = card.numbers.map((x) => x === 0 || calledSet.has(x));
          if (checkWin(card.numbers, serverMarked)) winners.push(card);
        }

        if (winners.length) {
          // If two cards complete on the same ball, one is picked at random.
          const winner = winners[crypto.randomInt(winners.length)];
          stopTimer(roundId); // stop first so no further ball is called
          await settleWin(r, cards.length, winner);
        }
      } catch (err) {
  ...
  // Returns true only if THIS call finished the round. The update is
  // conditional on status = 'playing', so if the auto-caller and a manual
  // claim race, exactly one of them pays out.
  async function finishRound(round, winnerTelegramId, winAmount) {
    stopTimer(round.id);
    const { data: updated } = await db.from('bingo_rounds').update({
      status: 'finished', winner_telegram_id: winnerTelegramId, finished_at: new Date().toISOString(),
    }).eq('id', round.id).eq('status', 'playing').select('id');
  ...
  // Pays out and finishes a round for a given winning card.
  async function settleWin(round, playerCount, card) {
    const { data: botRow } = await db.from('bots').select('bingo_payout_pct').eq('id', round.bot_id).maybeSingle();
    const pool = Number(round.stake) * playerCount;
    const payoutPct = Number((botRow && botRow.bingo_payout_pct) ?? 80) / 100;
    const winAmount = Math.floor(pool * payoutPct);

    const finished = await finishRound(round, card.telegram_id, winAmount);
    if (finished) await db.from('bingo_cards').update({ won: true }).eq('id', card.id);
    return { finished, winAmount };
  }

  // Winner details for the announcement: Telegram username + winning card #.
  async function winnerInfo(round) {
    if (!round || !round.winner_telegram_id) return null;
    const [{ data: u }, { data: c }] = await Promise.all([
      db.from('bot_users').select('username,first_name')
        .eq('bot_id', round.bot_id).eq('telegram_id', round.winner_telegram_id).maybeSingle(),
      db.from('bingo_cards').select('card_no')
        .eq('round_id', round.id).eq('telegram_id', round.winner_telegram_id).maybeSingle(),
    ]);
    return {
      telegramId: round.winner_telegram_id,
      username: u && u.username ? '@' + u.username : null,
      name: (u && u.first_name) || null,
      cardNo: c ? c.card_no : null,
    };
  }
  ...
  app.get('/api/mini/:botId/bingo/state', async (req, res) => {
    ...
    try {
      ...
      const { data: user } = await db.from('bot_users').select('game_balance,balance')
      ...
      // A round that just ended with a winner: everyone polling this stake
      // gets the winner's username + card so the client can show it for 3s.
      let result = null;
      const justFinished = await getRecentFinishedRound(bot.id, stake);
      if (justFinished) {
        const w = await winnerInfo(justFinished);
        const { count: finishedPlayers } = await db.from('bingo_cards')
          .select('id', { count: 'exact', head: true }).eq('round_id', justFinished.id);
        const pct = Number(bot.bingo_payout_pct ?? 80) / 100;
        result = {
          roundId: justFinished.id,
          finishedAt: justFinished.finished_at,
          winner: w,
          prize: Math.floor(Number(justFinished.stake) * (finishedPlayers || 0) * pct),
          lastNumbers: justFinished.called_numbers.slice(-1),
        };
      }
      ...
      res.json({
        ...
        stake,
        balance: Number((user && user.game_balance) || 0) + Number((user && user.balance) || 0),
        round: round ? {
          id: round.id,
          status: round.status,
  ...
  app.post('/api/mini/:botId/bingo/join', async (req, res) => {
    ...
    try {
      ...
      // Registration bonus is playable: if the game wallet can't cover the
      // stake, move just the shortfall over from the bonus balance first.
      const { data: wallet } = await db.from('bot_users').select('balance,game_balance')
        .eq('bot_id', bot.id).eq('telegram_id', a.user.id).maybeSingle();
      if (wallet) {
        const shortfall = stake - Number(wallet.game_balance);
        if (shortfall > 0 && Number(wallet.balance) >= shortfall) {
          await db.from('bot_users').update({
            balance: Number(wallet.balance) - shortfall,
            game_balance: Number(wallet.game_balance) + shortfall,
      ...
      const { data: after } = await db.from('bot_users').select('balance,game_balance')
        .eq('bot_id', bot.id).eq('telegram_id', a.user.id).maybeSingle();
      const playable = after ? Number(after.balance) + Number(after.game_balance) : Number(jr.new_balance);

      res.json({ ok: true, roundId: round.id, joinsInSeconds: JOIN_WINDOW_MS / 1000, newBalance: playable });
    } catch (err) {
  ...
  app.post('/api/mini/:botId/bingo/claim', async (req, res) => {
    ...
    try {
      ...
      const { count: playerCount } = await db.from('bingo_cards')
        .select('id', { count: 'exact', head: true }).eq('round_id', roundId);
      const { finished, winAmount } = await settleWin(round, playerCount || 0, card);
      if (!finished) return res.status(409).json({ error: 'This round already ended.' });

...
app.set('botRegistry', registry); // so bingo.js / wallet-bot.js can notify users
...
/* ------------------------------------------------------------------ */
/* Main menu (reply keyboard) shown after registration                 */
/* ------------------------------------------------------------------ */
// Guide, Rules, Help, Promo code and Language are deliberately not included.
// The label text is what Telegram sends back when the user taps a button,
// so these strings are matched exactly.
// Play/Deposit/Withdraw button labels live in wallet-bot.js (MENU_EXTRA) and
// are appended below via walletBot.extendedMenu().
const MENU = {
  profile: '👤 ፕሮፋይል',
  balance: '💰 ሂሳብ',
  invite: '🔗 ጋብዝ & አግኝ',
  promote: '📣 ድርጅቱን አስተዋውቅ',
};

const mainMenu = () => walletBot.extendedMenu([
  [{ text: MENU.profile }, { text: MENU.balance }],
  [{ text: MENU.invite }, { text: MENU.promote }],
]);

// Gives a user a referral code if they don't have one yet (older users, or
// anyone created before the referral program existed). Returns the code.
async function ensureReferralCode(botId, user) {
  if (user.referral_code) return user.referral_code;
  const code = await uniqueReferralCode(botId);
  await db.from('bot_users').update({ referral_code: code })
    .eq('bot_id', botId).eq('telegram_id', user.telegram_id);
  return code;
}

// How many people this user invited, and how many of them finished registering.
async function inviteCounts(botId, telegramId) {
  const { data, error } = await db.from('bot_users')
    .select('bonus_given').eq('bot_id', botId).eq('referred_by', telegramId);
  if (error) throw error;
  const rows = data || [];
  return { invited: rows.length, completed: rows.filter((u) => u.bonus_given).length };
}
...
function attachHandlers(entry) {
  ...
  // Menu buttons arrive as ordinary text messages. Only registered users
  // (bonus claimed) get answers; anyone else is pointed back to /start.
  bot.on('message', async (msg) => {
    const text = (msg.text || '').trim();
    const r = row();

    // Deposit/Withdraw/Play + their multi-step conversation replies.
    // Checked for ANY registered user, since a bare number (deposit amount,
    // phone) won't match the fixed MENU labels below.
    try {
      const { data: existingUser } = await db.from('bot_users').select('*')
        .eq('bot_id', r.id).eq('telegram_id', msg.from.id).maybeSingle();
      if (existingUser && existingUser.bonus_given) {
        const handled = await walletBot.handleWalletMessage(bot, r, msg, existingUser);
        if (handled) return;
      }
    } catch (err) {
      console.error('wallet message error:', err);
    }

    if (!Object.values(MENU).includes(text)) return;
    try {
      const { data: user } = await db.from('bot_users').select('*')
        .eq('bot_id', r.id).eq('telegram_id', msg.from.id).maybeSingle();

      if (!user || !user.bonus_given) {
        return bot.sendMessage(msg.chat.id, 'Please send /start and finish registration first.');
      }
      const chatId = msg.chat.id;

      if (text === MENU.profile) {
        const name = md(user.first_name || user.username || 'User');
        const joined = user.created_at ? String(user.created_at).slice(0, 10) : '-';
        return bot.sendMessage(chatId,
          `👤 *Profile*\nName: ${name}\n` +
          `${user.username ? `Username: @${md(user.username)}\n` : ''}` +
          `ID: \`${user.telegram_id}\`\nJoined: ${joined}`,
          { parse_mode: 'Markdown', reply_markup: mainMenu() });
      }
      ...
      if (text === MENU.balance) {
        return bot.sendMessage(chatId,
          `💰 *Balance*\n*${Number(user.balance)} birr*`,
          { parse_mode: 'Markdown', reply_markup: mainMenu() });
      }

      // Invite & Promote both need the referral program to be on.
      if (!(Number(r.referral_bonus) > 0 && r.bot_username)) {
        return bot.sendMessage(chatId, 'Invitations are not available right now.', { reply_markup: mainMenu() });
      }
      const code = await ensureReferralCode(r.id, user);
      const link = referralLink(r.bot_username, code);

      if (text === MENU.invite) {
        const c = await inviteCounts(r.id, user.telegram_id);
        return bot.sendMessage(chatId,
          `🔗 *Invite & Earn*\n` +
          `Earn *${Number(r.referral_bonus)} birr* for every friend who joins and claims their bonus.\n\n` +
          `Your link:\n${link}\n\n` +
          `Invited: *${c.invited}*\nCompleted: *${c.completed}*`,
          { parse_mode: 'Markdown', disable_web_page_preview: true, reply_markup: mainMenu() });
      }

      if (text === MENU.promote) {
        const shareText = `Join and get a bonus: ${Number(r.bonus)} birr`;
        return bot.sendMessage(chatId,
          `📣 *Promote*\nTap the button to send your invite link to friends or groups.`,
          {
            parse_mode: 'Markdown',
  ...
  bot.on('callback_query', async (q) => {
    ...
    try {
      if (await walletBot.handleWalletCallback(bot, row(), q)) return;
      ...
      // A message can carry only one kind of keyboard, so the menu goes in
      // its own short follow-up message.
      await bot.sendMessage(chatId, 'Use the menu below.', { reply_markup: mainMenu() });
    } catch (err) {
...
/* ------------------------------------------------------------------ */
let channels = { handleMyChatMember: async () => {} }; // replaced once mounted below
app.post('/telegram/webhook/:id', (req, res) => {
  ...
  if (req.body && req.body.my_chat_member) {
    channels.handleMyChatMember(entry.row.id, req.body.my_chat_member);
  }
...
/* ------------------------------------------------------------------ */
/* Bingo + wallet (deposit/withdraw) modules                           */
/* ------------------------------------------------------------------ */
const walletBot = require('./wallet-bot')(app, db, { requireAdmin, MINIAPP_BASE });
require('./bingo')(app, db, { requireAdmin, miniAuth, mainMenuFor: mainMenu, sendMessage: null });
channels = require('./channels')(app, db, {
  requireAdmin,
  getEntry: (id) => registry.get(Number(id)),
});
...
const publicBot = (b, stats = {}) => ({
  ...
  bingo_name: b.bingo_name || 'Bingo',
  bingo_enabled: b.bingo_enabled !== false,
  bingo_stakes: b.bingo_stakes || [10, 20, 50, 100],
  bingo_payout_pct: Number(b.bingo_payout_pct ?? 80),
  deposit_phone: b.deposit_phone || '0930008319',
  withdraw_min_deposit: Number(b.withdraw_min_deposit ?? 100),
...
app.patch('/api/admin/bots/:id', requireAdmin, async (req, res) => {
  ...
  const allowed = [
    'name', 'channel', 'group_chat', 'bonus', 'verify', 'active', 'continue_url', 'continue_text',
    'referral_bonus', 'require_all_clicks',
    'bingo_name', 'bingo_enabled', 'bingo_stakes', 'bingo_payout_pct', 'deposit_phone', 'withdraw_min_deposit',
  ];
  const patch = {};
  ...
  if ('bingo_name' in patch) {
    patch.bingo_name = String(patch.bingo_name || 'Bingo').trim().slice(0, 40) || 'Bingo';
  }
  if ('bingo_enabled' in patch) patch.bingo_enabled = patch.bingo_enabled !== false;
  if ('bingo_stakes' in patch) {
    const stakes = (Array.isArray(patch.bingo_stakes) ? patch.bingo_stakes : String(patch.bingo_stakes).split(','))
      .map((n) => Number(n)).filter((n) => n > 0);
    if (!stakes.length) return res.status(400).json({ error: 'At least one stake amount is required' });
    patch.bingo_stakes = stakes;
  }
  if ('bingo_payout_pct' in patch) {
    const pct = Number(patch.bingo_payout_pct);
    if (!(pct > 0 && pct <= 100)) return res.status(400).json({ error: 'Payout percent must be between 1 and 100' });
    patch.bingo_payout_pct = pct;
  }
  if ('deposit_phone' in patch) {
    patch.deposit_phone = String(patch.deposit_phone || '').trim().slice(0, 20) || '0930008319';
  }
  if ('withdraw_min_deposit' in patch) {
    patch.withdraw_min_deposit = Math.max(0, Number(patch.withdraw_min_deposit) || 0);
  }

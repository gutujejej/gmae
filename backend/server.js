...
// Signed, tamper-proof click link. Telegram URL buttons can't tell the bot
// they were tapped, so each one points at our own /go endpoint, which records
// the click and then redirects to the real destination.
function clickToken(botId, telegramId, key) {
  const body = Buffer.from(JSON.stringify({ b: botId, u: telegramId, k: key })).toString('base64url');
  const sig = crypto.createHmac('sha256', JWT_SECRET).update(`click:${body}`).digest('base64url').slice(0, 22);
  return `${body}.${sig}`;
}

function readClickToken(token) {
  try {
    const [body, sig] = String(token).split('.');
    const expected = crypto.createHmac('sha256', JWT_SECRET).update(`click:${body}`).digest('base64url').slice(0, 22);
    if (!sig || sig.length !== expected.length ||
        !crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expected))) return null;
    const p = JSON.parse(Buffer.from(body, 'base64url').toString());
    return { botId: Number(p.b), telegramId: Number(p.u), key: String(p.k) };
  } catch { return null; }
}

// Telegram caps inline-button url length; if a tracked link would be too long
// we fall back to the direct URL (that button then can't be enforced).
function trackedUrl(row, telegramId, btn) {
  const u = `${PUBLIC_URL.replace(/\/$/, '')}/go/${clickToken(row.id, telegramId, btn.key)}`;
  return u.length <= 512 ? u : btn.url;
}

function keyboard(row, telegramId) {
  const track = row.require_all_clicks !== false;
  return {
    inline_keyboard: [
      ...requiredButtons(row).map((b) => [{
        text: b.text,
        url: track ? trackedUrl(row, telegramId, b) : b.url,
      }]),
...
function attachHandlers(entry) {
  ...
  bot.onText(/\/start(?:\s+(\S+))?/, async (msg, match) => {
    try {
      ...
      const steps = hasButtons
        ? (r.require_all_clicks !== false
            ? `You must open *every* button below, then tap *${cont}* to receive your *${Number(r.bonus)} birr* registration bonus.`
      ...
      await bot.sendMessage(
        ...
        { parse_mode: 'Markdown', reply_markup: keyboard(r, msg.from.id) }
      );
    } catch (err) {
  ...
  bot.on('callback_query', async (q) => {
    ...
    try {
      ...
      if (r.require_all_clicks !== false) {
        const missing = await unclickedButtons(r, q.from.id);
        if (missing.length) {
          const names = missing.map((b) => `• ${b.text}`).join('\n');
          const text = `Please open all buttons first (${missing.length} left):\n${names}`;
          return bot.answerCallbackQuery(q.id, {
            text: text.length > 200 ? `Please open all buttons first (${missing.length} left).` : text,
            show_alert: true,
          });
        }
      }
...
// Click tracker: records that this user opened this button, then sends them on
// to the real destination. The token is signed, so users can't forge clicks
// for buttons they haven't opened or for other people.
app.get('/go/:token', async (req, res) => {
  const t = readClickToken(req.params.token);
  if (!t) return res.status(400).send('Invalid link.');

  const entry = registry.get(t.botId);
  if (!entry) return res.status(404).send('This bot is not available.');

  const btn = requiredButtons(entry.row).find((b) => b.key === t.key);
  if (!btn) return res.status(404).send('This button no longer exists.');

  // Record the click, but never block the redirect if the write fails.
  const { error } = await db.from('button_clicks').upsert(
    { bot_id: t.botId, telegram_id: t.telegramId, button_key: t.key },
    { onConflict: 'bot_id,telegram_id,button_key', ignoreDuplicates: true }
  );
  if (error) console.error('click record failed:', error.message);
